...
const GetManyFeedbackTopicCategoryTopicSchema = v.array(GetFeedbackTopicCategoryTopicSchema);

const feedbackTopicCategoryTopicContract = oc.tag("Categories & Topics").prefix("/topic_category_topics").router({
  all: oc
    .route({
      method: "GET",
      path: "/",
      summary: "List topic-categories pairs",
      description: "Get topic-categories pairs filtered by category id or topic id",
    })
    .input(
      v.optional(v.object({
